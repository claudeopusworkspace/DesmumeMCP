...
Launched as a subprocess by the main MCP server:
    python -m desmume_mcp.renderer --journal-sock <path> --rom <path> [--initial-state <path>] --port <port>

Initializes its own DeSmuME instance, attaches the HLS streamer, and
...
logger = logging.getLogger("desmume_mcp.renderer")


def _ensure_headless_env() -> None:
    """Set SDL env vars for headless operation."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DeSmuME rendering emulator")
...
def main() -> None:
    ...
    # Import after env setup so SDL picks up dummy drivers
