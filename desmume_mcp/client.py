"""Client for connecting to the running DeSmuME MCP bridge from external scripts.

Usage:
    from desmume_mcp.client import connect

    emu = connect()  # or connect("/path/to/.desmume_bridge.sock")
...
def connect(socket_path: str | None = None) -> EmulatorClient:
    """Connect to the running DeSmuME MCP bridge.

    Args:
        socket_path: Path to the Unix socket. If not provided, searches for
                     .desmume_bridge.sock in common locations (CWD, parent dirs).
    ...
    search = [
        Path.cwd() / ".desmume_bridge.sock",
        Path.cwd() / "DesmumeMCP" / ".desmume_bridge.sock",
    ]
    # Also check DESMUME_BRIDGE_SOCK env var
    ...
    raise FileNotFoundError(
        ...
        "Is the MCP server running? Set DESMUME_BRIDGE_SOCK or pass the path explicitly."
    )
